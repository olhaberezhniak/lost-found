  ...
  async (req, res) => {
    //try {
      ...
      if (req.body.files.length > 0) {
        const destination = `Diploma/users/${newProduct.posted_by}/lost-found/${newProduct._id}`;
...
router.put("/edit-lost-found-product",
  upload.array("files", 5),
  UserAuth,
  ValidateLostFound,
  async (req, res) => {
    try {
      ...
      if (toUpload > 0) {
        ...
        const destination = `Diploma/users/${product.posted_by}/lost-found/${product._id}`;
...
router.delete("/delete-lost-found-product", UserAuth, async (req, res) => {
  try {
    ...
    await DeleteAFolder(
      `Diploma/users/${product.posted_by}/lost-found/${product._id}`
    );
