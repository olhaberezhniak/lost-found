...
const {
  ...
  get_encoded_data,
  get_dashboard_stats,
} = require("../helpers/DashBoardHandler");
...
router.post(
  ...
  /*ValidateRegister,*/ async (req, res) => {
    try {
      ...
      const destination = `Diploma/users/${newUser._id}/profile_picture`;
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // Destination for profile_picture
      const destination = `Diploma/users/${user_id.toString()}/profile_picture`;
