...
function Modal({ toggleModal, modal }) {
  ...
  const newBoughtTime = getIsoTime({ date: boughtTime });
  const newWarranty = getIsoTime({ date: warranty });

  let status;
  let message;

  status = useSelector((state) => state.buySell.addItemsResponse);
  const errorMessage = useSelector(
    (state) => state.buySell.errorMessageBuySell
  );
          ...
          <div className="modal-content">
            ...
            <form className="form02">
              ...
              <div>
                ...
                <input
                  onChange={(e) => setBoughtTime(e.target.value)}
                  type="date"
